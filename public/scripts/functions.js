...
export function loadCartItem(cartItems, container) {
  container.innerHTML = "";  // Limpa qualquer conteúdo anterior

  if (!cartItems || cartItems.length === 0) {
      container.innerHTML = "<p>Não há itens no carrinho.</p>";
      return;
  }
  ...
  cartItems.forEach(item => {
      ...
      // Adiciona a funcionalidade para remover item
      div.querySelector(".remove-item").addEventListener("click", () => removeCartItem(item));
  });
}
...
export function removeCartItem(itemToRemove) {
  ...
  window.location.reload(); // Ou você pode atualizar a UI manualmente sem recarregar a página
}
